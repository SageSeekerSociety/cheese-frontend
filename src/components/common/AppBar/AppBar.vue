    ...
    <v-app-bar app flat>
        <v-container class="mx-auto d-flex align-center justify-center">
            ...
            <v-responsive max-width="240" class="me-4">
                <v-text-field density="compact" flat hide-details label="搜索" prepend-inner-icon="mdi-magnify" rounded="lg"
                    single-line variant="solo-filled"></v-text-field>
            </v-responsive>
            <v-btn icon class="me-4">
                <v-icon>mdi-bell</v-icon>
            </v-btn>
            <v-menu open-on-hover location="bottom center" :offset="16" :close-on-content-click="false" v-if="loggedIn">
...
const hasAvatar = computed(() => !!AccountService._user.value?.avatar && AccountService._user.value?.avatar != 'default.jpg' && AccountService._user.value?.avatar != 'deafult.jpg')
